...
print("LOADING NEW bandpass_helpers.py")
...
def get_cut_stat_band(band, fourier_band_filter, out_of_band_filter,
                 take_sqrt=False):
    '''get cut statistic which is a rough S/N estiamte of the band.'''
    if np.std(band) > 1e-4:
        # interferogram = get_cleaned_interferogram(data, 5, 10, 7,
        ...
        # Just take a conv between the filter of ones and our data I guess...
        signal_stat = np.mean(band[fourier_band_filter])
        noise_stat = np.std(band[out_of_band_filter])
        return (signal_stat / noise_stat)
    else:
        print('np.std(band) < 1e-4')
...
def get_cleaned_interferogram(data, n_rms_iters, spike_threshold, poly_order, max_ind=None,
                              plot=False, polyfit=False, take_sqrt=False, center_interf=False, max_ind_dashed_line=0, figsave=0):
    ...
    if (plot):
        ...
        ax[0].plot(data, label=f'raw interf {len(data)}')
        ax[0].plot(corrected_interferogram, label=f'corrected interf {len(corrected_interferogram)}')

    if (center_interf):
        # uncommenting this out - ss, 4.20.26
        centered_interferogram, max_ind = center_interferogram(corrected_interferogram,
                                                  max_ind=max_ind)
    # commenting this out - ss, 4.20.26
    else: 
    ...
    if (plot):
        ax[1].plot(window, label=f'window {len(window)}')
        ax[0].plot(centered_interferogram, label=f'centered interf {len(centered_interferogram)}')
        ax[0].axvline(len(data)/2, ls='--')
        ax[1].axvline(len(window)/2, ls='--')
        if max_ind_dashed_line != 0:
            ax[0].axvline(max_ind_dashed_line, ls='--', color='r', label=f'max_ind = {max_ind_dashed_line}')
        ax[0].legend()
        ax[1].legend()
        if figsave != 0:
            plt.savefig(figsave) # should include file path, name, and type, as in .png
        plt.show()
    ...
    return phase_corrected_passband[:int(np.ceil(len(
        phase_corrected_passband) / 2))], max_ind
...
def plot_band_snr_values(band_attrs, band):
    ...
    # adding color bar
    sc = plt.scatter(band_centers, bandwidths, s=3 * (snr_values), c=np.sqrt(snr_values))
    plt.colorbar(sc, label='sqrt(SNR)')
...
def plot_colored_hist(data, bins, snr_values, n_rms_iters=7, rms_threshold=5,
    ...
    # SS: added the lines below to replace the one above. 1-12-2026
    ax = plt.gca()
    sm = matplotlib.cm.ScalarMappable(norm=norm, cmap=cmap)
    sm.set_array([])  # required for older Matplotlib versions

    plt.colorbar(sm, ax=ax, label='mean SNR per bin')
...
def find_bands_clean(data, fourier_band_filters, fourier_noise_filter, take_sqrt = False, divide_by_nonmax = False):
    '''
    get initial cut stats for all the bands
    basically the SNRs of each band
    '''
    ...
    total_cut_stats = []
    total_cut_stats.clear()
    
    for i in range(len(data)):
        if (np.std(data[i]) > 1e-4):
            cut_stats = [get_cut_stat_band(data[i], f, fourier_noise_filter,
                                      take_sqrt=take_sqrt)
    ...
    cut_stats_combined = np.sqrt(np.sum(np.array(total_cut_stats) ** 2, axis=1))
    return (total_cut_stats, cut_stats_combined)
...
def center_interferogram(interferogram, max_ind=None):
    ...
    return centered_interferogram, max_ind
    # return interferogram[max_ind - interferogram_size: (
    #     max_ind + interferogram_size + 1)]

    # return interferogram # ss commented out 4.22.26
...
def invert_interferogram(interferogram, window):
    # plt.figure()
    # plt.title('invert interferogram \n 1. interferogram')
    # plt.plot(interferogram)
    # plt.show()
    fft_interferogram = np.fft.fft(interferogram * window)
    # plt.figure()
    # plt.title('2. fft_interferogram')
    # plt.plot(fft_interferogram)
    # plt.show()
    ...
    # plt.plot(phase_spectrum, label='wrapped')
    # plt.show()
    
    phase_spectrum = np.unwrap(phase_spectrum)
    # plt.plot(phase_spectrum, label='unwrapped')
    # plt.legend()
    # plt.show()
    ...
    phase_corrected = fft_interferogram * np.exp(-1j * phase_spectrum)
    # plt.figure()
    # plt.title('4. phase_corrected??')
    # plt.plot(phase_corrected, label='phase corrected')
    # plt.show()
    return phase_corrected


def frequency(raw_passband, c, fts_stage_step_size, fts_frequency_cal, prints=False):
    ...
    if (prints):
        print(f'Input, fts_stage_step_size {fts_stage_step_size}')
        print(f'Input, fts_frequency_cal {fts_frequency_cal}')
        print(f'N_samples_kept: {N_samples_kept}')
...
def get_beam_splitter_spectrum(f_hz):
    '''
    Get correction function for the beam splitter reflectance.
    Material: LDPE
    Thickness: 25um
    Angle of incidence: 34.5 degrees
    '''
    # frequency --> wavelenghts
    # f_hz = f_hz.to(u.Hz)
    f_wavelengths = c / f_hz

    # complex index of refraction
    n, eps = 1.5141,2.2923 # LDPE
    k= 1j*np.emath.sqrt(n**2 -eps)
    N = n + 1j*k
    # print(N)

    # input params
    d_ldpe = 25.4e-6 # thickness in m, 25.4um
    theta_i = np.radians(34.5)

    R_25um_ldpe = reflectance_s_polarized_complex(1, N, d_ldpe, theta_i, f_wavelengths) # 25.4 um thickness
    return R_25um_ldpe

def process_given_band_and_get_attrs(passband, frequency_hz,
                 bin_min_freq=15, lower_bound=22., upper_bound=35.,
                 slope_cut=1e-3, end_fit_freq=23,
                 ...
                 transfer_func_edges=(None, None), beamsplitter=True, edges_prints=False, edges_plots=False):
    '''
    lower bound and upper bound are frequencies
    I created this for the HWP bands, which are to be passed in as band = np.sqrt(bandQ**2 + bandU**2)
    '''
    bin_min = find_freq(frequency_hz, bin_min_freq) # outputs index of frequency closest to desired value
    
    if noise_bounds[1] is None:
        # print(f'noise bounds: {noise_bounds[0]}, {frequency_hz[-1]/1e9} GHz')
        noise_bounds = (noise_bounds[0], frequency_hz[-1] / 1e9) # noise_bounds are computed as going from (start, None) start to end of array
    
    # plt.figure()
    # plt.title('Plot 1 - starting band')
    # plt.plot(frequency_hz/1e9, passband)
    # plt.axvline(bin_min_freq)
    # plt.axvline(noise_bounds[0], ls='--', color='k')
    # plt.axvline(frequency_hz[-1] / 1e9, ls='--', color='k')
    # plt.show()
    
    # print(f'fit_noise: {fit_noise}')
    if fit_noise:
    ...
    # print(f'correction_func: {correction_func}')
    ...
    noise_start_index = find_freq(frequency_hz, noise_bounds[0]) # index of the noise start freq
    noise_end_index = find_freq(frequency_hz, noise_bounds[1]) # index of the noise end freq
    
    # subtract a mean baseline from the band
    # print(f'subtract_mean: {subtract_mean}')
    if (subtract_mean):
        passband = passband - np.mean(passband[
            noise_start_index:noise_end_index]) # the mean that is subtracted is calculated in the noise region

    # plt.figure()
    # # plt.title('Plot 2 - after mean subtraction')
    # plt.plot(frequency_hz/1e9, passband / np.nanmax(passband), label='no beamsp correct')
    # plt.axvline(bin_min_freq)
    # plt.axvline(noise_bounds[0], ls='--', color='k')
    # plt.axvline(noise_bounds[1], ls='--', color='k')
    # # plt.show()

    # before normalizing the band, you should do the beam splitter correction
    # beam splitter correction important to do before getting band attrs
    # print(f'beamsplitter: {beamsplitter}')
    if (beamsplitter):
        print(f'Correcting for beam splitter')
        beam_splitter_spectrum = get_beam_splitter_spectrum(frequency_hz)
        passband = passband / beam_splitter_spectrum

    # plt.plot(frequency_hz/1e9, passband, color='r')
    # plt.ylim(0, 100)
    # plt.show()
    ...
    # plt.figure()
    # # plt.title('Plot 3 - Normed band')
    # plt.plot(frequency_hz/1e9, normed_passband, label='beamsp corrected')
    # plt.ylim(0, 1.2)
    # plt.legend()
    # plt.show()

    # calculate the band center and bandwidth and edges
    # print(f'Inside function get_passband, slope_cut value sent into get_band_attrs: {slope_cut}')
    # slope cut value here is the default of get_passband, 1e-3
    # print('These are the arguments for the get_band_attrs:')
    # print(f'lower_bound {lower_bound} GHz')
    # print(f'upper_bound {upper_bound} GHz')
    # print(f'slope cut {slope_cut}')
    # print(f'bin_min {bin_min} index of min freq, {bin_min_freq}')
    cf, bw, lower_edge, upper_edge = get_band_attrs(
        normed_passband, frequency_hz, lower_bound, upper_bound, slope_cut,
        bin_min=bin_min, edges_prints=edges_prints, edges_plots=edges_plots)
    cf = np.floor(cf*100)/100. # rounding to two decimal points
    bw = np.floor(bw*100)/100.

    if (lower_edge is None):
        print('lower edge not fitted...')
        return normed_passband, None, None, None, None, None, None, frequency_hz
    ...
    # calculate the SNR
    snr = np.around(
        1 / np.std(normed_passband[noise_start_index:]), decimals=1) # can do this because normed passband
    return passband, cf, bw, snr, lower_edge, upper_edge, bin_min, frequency_hz # added lower index (bin_min)
...
def get_passband(interferogram, fts_stage_step_size, fts_frequency_cal,
                 ...
                 subtract_mean=True, edge_power_limit=.05, normalize=True,
                 amplitude_transfer_func=None, interp_freqs=None,
                 transfer_func_edges=(None, None), beamsplitter=True):
    '''
    lower bound and upper bound are frequencies
    '''
    print('In get_passband, add center_interferogram lines!')
    if (max_ind is None):
    ...
    # correct interferogram by removing average, despiking, and removing polynomial
    corrected_interferogram = correct_interferogram(
        interferogram, n_rms_iters, spike_threshold, poly_order,
        polyfit=True)
    ...
    bin_min = find_freq(frequency_hz, bin_min_freq) # outputs index of frequency closest to desired value
    # print(f'output bin_min from find_freq: {bin_min}')
    # print(f'frequency at bin_min index: {frequency_hz[bin_min] / 1e9} GHz')
    ...
    normed_passband = normalize_passband(passband, lower_index=bin_min) # should be inside "if..."?
    if (normalize):
        passband = normed_passband
    ...
    if (lower_edge is None):
        print('lower edge not fitted...')
        plt.figure()
        plt.plot(frequency_hz, normed_passband)
        plt.show()
        return None, None, None, None, None, None, frequency_hz
...
def plot_band_data(run_num, passbands, band_attrs, average_band, frequencies,
    ...
    low_SN = np.where(np.sqrt(weights) < 15)
    high_SN = np.where(np.sqrt(weights) > 15)

    plot_indiv_bands(axes[1], passbands[low_SN], band_attrs[low_SN],
                     frequencies, bin_min_freq, plot_freq_range, '< 15')
...
# generally have relabeled some names in this function for my understanding - SS
def obtain_passbands(
        band_num, data_sets, total_good_inds, fts_step_size,
        fts_frequency_cal, output_vals=False, bin_min_freq=15,
        plot_freq_range=None, low_snr_cutoff=15, high_snr_change=80,
        high_snr_cutoff=1e10, plots=True, **passband_kwargs): # here, total_good_inds is replacing total_good_band_channels
    
    # print("Inside function obtain passbands, this is band_num (band type, 0 (90GHz) 1 (150GHz):", band_num)
    average_bands = [] # so the average per run?
    total_passbands = [] # as in the bands for each det?
    total_band_attrs = []
    frequencies = None
    
    for i, data_set in enumerate(data_sets):
        # print(f'data_sets[{i}]')
        passbands = [] # empty list of passbands-ss
        band_attrs = [] # empty list of band attributes (center, width, snr, lower and upper edges)-ss

        good_inds = total_good_inds[i][band_num] # get the indices for the good dets for run i, band type = band_num -ss
        for det_idx in good_inds: # here det_idx replaces channel
            frequency_calibration_factor = fts_frequency_cal
            amplitude_transfer_func = None
            common_frequencies = None
            interferogram = data_set[:, det_idx]
            ...
            # by this point I should have looked at the scatter plot of the band SNRs and chosen the appropriate cutoff from that.-ss
            # should the SNR cutoff be different per run? Right now it isn't.-ss
            if passband is not None and (
                ...
                if (snr > high_snr_change): # replace SNR of really high SNR dets with a reasonable maximum (kind of want to omit this,so just set high_snr_change to be really large)
                    snr = high_snr_change

                passbands.append(passband)
                # print(f"saved passband for det {det_idx}")
                band_attrs.append([det_idx, center_freq, bin_width, snr,
                                   low_edge, upper_edge])
                # print(f'saved attrs for det {det_idx}')
        ...
        if len(passbands) > 0:
            ...
            bin_min = find_freq(frequencies, bin_min_freq)
            average_band = average_band / np.max(average_band[bin_min:]) # peak normalize
        else:
            average_band = None
...
def find_integration_limits(passband, frequency, lower_start_frequency,
    ...
    '''
    lower_start = find_freq(frequency, lower_start_frequency) # index of lower start frequency of the band edge
    upper_start = find_freq(frequency, upper_start_frequency) # index of the upper start frequency of the band edge
    # print(f'lower_start, upper_start (inds): {lower_start}, {upper_start}')
    # print(f'lower_start, upper_start (freqs Hz --> GHz): {frequency[lower_start]/1e9}, {frequency[upper_start]/1e9}')
    # print(f'slope cut: {slope_cut}')
    ...
    # code below is getting the slopes at the outer ends of the in-band region and moving inward until the band edges are
    # found as defined by the slope_cut
    try:
        while ((passband[int(lower_start+2)]-passband[int(lower_start)])/(
            ...
            lower_start += 1
            # print(f'lower_start: {lower_start}')
        while ((passband[int(upper_start)]-passband[int(upper_start-2)])/((
            ...
            upper_start -= 1
            # print(f'upper_start: {upper_start}')
        
    except IndexError:
        print("index error when finding integration limits..")
        return int(lower_start_orig), int(upper_start_orig)

    # the resulting lower and upper band edge indices
    lower, upper = lower_start, upper_start

    # # plotting just for visualization..-ss
    # plt.figure()
    # plt.plot(frequency/1e9, passband)
    # plt.axvline(frequency[lower_start_orig]/1e9, color='grey', alpha=0.5, ls='--')
    # plt.axvline(frequency[lower_start_orig+2]/1e9, color='k')
    # plt.axvline(frequency[lower]/1e9, color='r', alpha=0.5, ls='--')
    
    # plt.axvline(frequency[upper_start_orig]/1e9, color='grey', alpha=0.5, ls='--')
    # plt.axvline(frequency[upper_start_orig-2]/1e9, color='k')
    # plt.axvline(frequency[upper]/1e9, color='r', alpha=0.5, ls='--')
    # plt.show()
...
def integrate_bands(frequency_range, passband_range, spectral_index):
    ...
    # ####
    # # just plotting what we are integrating over
    # plt.figure()
    # y_top_exp = -1. + spectral_index # = 1
    # y_bottom_exp = -2. + spectral_index # = 0
    # cf = top/bottom
    # y_top = frequency_range**(-1. + spectral_index) * passband_range
    # y_bottom = frequency_range**(-2. + spectral_index) * passband_range
    # plt.plot(frequency_range, passband_range, label='normal band')
    # plt.plot(frequency_range, y_top, label='top')
    # plt.plot(frequency_range, y_bottom, label='bottom')
    # plt.axvline(cf, color='k')
    # plt.semilogy()
    # plt.legend()
    # plt.show()

    # # print the frequency array exponents
    # print(f'y_top_exp {y_top_exp}')
    # print(f'y_bottom_exp {y_bottom_exp}')
...
def return_cent(passband, frequency, lower_bound_frequency,
    ...
    spectral_index: [2]
    '''
    # get the indices of the upper and lower band edges
    # print(f'Inside of return_cent:')
    # print(f'Lower and upper bound frequencies (presumably) for the band region')
    # plt.figure()
    # plt.plot(frequency, passband)
    # plt.axvline(lower_bound_frequency)
    # plt.axvline(upper_bound_frequency)
    # plt.ylim(0, 1.2)
    ...
    # get center of passband
    # plt.axvspan(frequency[int(lower)], frequency[int(upper)])
    # plt.plot(frequency[int(lower):int(upper)], passband[int(lower):int(upper)], color='k')
    # plt.show()
    centers = [integrate_bands(frequency[int(lower):int(upper)], passband[int(
        lower):int(upper)], index) for index in spectral_index]
    # print(f'centers {centers[0]/1e9} GHz')

    # why is centers storing lower and upper indices?
    centers.append(lower)
    centers.append(upper)
    # print(f'centers after appending: {centers}')
    # print(f'\n Inside of return_cent, cf = {centers[0] / 1e9} GHz')
    return (centers)
...
def bandwidth(passband, frequency, lower, upper):
    ...
    calculates weighted band withs of # passband'''
...
def bootstrap_plot(average_band, all_bands, frequencies, weights, iterations,
    ...
    print(f'\n inside of bootstrap_plot')
    print(f'norm_start_freq {norm_start_freq}') # is this actually an index or is it a frequency?
    print(f'weights: {weights}')
    assert len(all_bands) > 0

    bootstrap_data = all_bands.T
    print(f'size and shape of bootstrap_data: {bootstrap_data.shape}')
    bootstrap_lower = []
    bootstrap_upper = []
    ...
    for i in range(len(bootstrap_data)):
        ...
        for _ in range(iterations):
            ...
            # print(f'{_}; resampled_data {resampled_data}; resampled_weights {resampled_weights}')
            # print(f'lenght of resampled_data {resampled_data.shape}, resasmpled weights size, {resampled_weights.shape}')
...
def run_through_bands(band_label, *passband_args, weight_func=np.square,
    ...
    print(f'Inside of run_through_bands')
    # print(f'this is the band_label: {band_label}')
    # print('These are passband_args:', *passband_args)
    # print(f'plots = {plots}')
    # print(f'run_plots = {run_plots}')
    # print(f'hist_cuts = {hist_cuts}')
    # print(f'hist_cut_plots = {hist_cut_plots}')
    # print(f'summary_plots = {summary_plots}')
    # print(f'bootstrap_confidence_level {bootstrap_confidence_level}')
    ...
    norm_start_freq = bin_min # min freq index
    print(f'norm_start_freq GHz: {frequencies[norm_start_freq]/1e9}')
    ...
    # this is just squaring the snrs
    ...
    upper, lower = bootstrap_plot(
        total_average_band, all_passbands, frequencies, weights, 5, # 5 used to 1000, I just want to see the individual iterations so i reduced
...
def get_band_attrs(band, frequencies, lower_bound, upper_bound, slope_cut,
                   bin_min=0, edges_prints=False, edges_plots=True):
    # get center freq, lower band edge index, and upper band edge index
    # I think lower_limit and upper_limit are indice
    ...
    # print(f'Inside of get_band_attrs:')
    center_freq /= 1e9
    # print(f'center_freq: {center_freq}GHz')
    ...
    # # just a plot to see
    # plt.figure()
    # plt.title('get_band_attrs plot')
    # plt.plot(frequencies/1e9, band)
    # plt.axvspan(center_freq - width/2., center_freq + width/2., alpha=0.5)
    # plt.ylim(0, 1.2)
    # # plt.show()
    
    # normalize the band before getting the edges
    # print(f'This is bin_min: {frequencies[bin_min] / 1e9}')

    # band that was passed through was already normalized
    normed_band = normalize_passband(band, lower_index=bin_min) # peak normalized
    
    # plt.figure()
    # plt.plot(frequencies, band, alpha=0.5)
    # plt.plot(frequencies, normed_band, alpha=0.5)
    # plt.ylim(0, 1.2)
    # plt.show()

    # I believe these are actually returned as frequencies
    # okay I'm going to require that a minimum frequency index is passed in to circumvent lower
    # freq bs blowing up due to beamsplitter correction
    lower_edge, upper_edge = get_band_edges(frequencies, normed_band, lower_index = bin_min, plot=edges_plots, prints=edges_prints)
    print(f'(lower_edge, upper_edge) {lower_edge}, {upper_edge}')
    # print(f'lower_edge, upper_edge: {lower_edge}, {upper_edge}GHz')
    # plt.axvline(lower_edge, color='k')
    # plt.axvline(upper_edge, color='k')
    # plt.show()
...
def get_band_edges_manually(passband, limit=0.05, lower_index = 0): # limit of 0.05 just means that the band response is that much by the time we're at the edge of the band
    
    # go until we hit .05
    passband = passband[lower_index:]
    current_index = np.argmax(passband)
    
    # get the low edge
    while (np.abs(passband[current_index]) > limit): # why is np.abs needed? Isn't the passband always positive?
        current_index += 1
    ...
    # # Plot just to see
    # print(f'\n Inside of get_band_edges_manually')
    # plt.figure()
    # plt.plot(passband)
    # plt.axhline(passband[end_index], label='high edge value')
    # plt.axhline(passband[start_index], label='low edge value')
    # plt.legend()
    # plt.show()
...
def fit_band_edge(frequencies, passband, edge_guess_ind, plot=False,
                  limit=.05):
    # print('Inside fit_band_edge')
    # go, say, 2 points below and 2 points above to fit this edge
    ind_limit = 4

    # print(f'edge_guess_ind {edge_guess_ind}')
    # fit a cubic polynomial to a really small region of frequency and passband around the band edge
    ...
    roots = roots.real
    print(f'roots {roots}')
    closest_root = min(roots, key=lambda x: abs(x - frequencies[edge_guess_ind]))
    # print('this is the closest_root ', closest_root)
    ...
    # if the fit is really off, just return the earlier guess.
...
def get_band_edges(frequencies, passband, limit=.05, lower_index = 0, plot=False, prints=False):
    # make sure this band is normalized.

    # print('inside of get_band_edges and removing lower freq stuff')
    passband = passband[lower_index:]
    frequencies = frequencies[lower_index:]
    # plt.figure()
    # plt.title('Band inside get_band_edges')
    # plt.plot(frequencies/1e9, passband, color='purple')
    # plt.show()
    if np.max(passband) != 1:
        print('Inside get_band_edges and unfortunately met the requirement: np.max(passband) != 1')
        # print('this is the lower index:', lower_index)
        return None, None
    ...
    if (plot):
        ...
        print(f'\n inside of get_band_edges, used get_band_edges_manually')
        plt.figure()
        plt.plot(frequencies/1e9, passband)
        plt.axvline(frequencies[start]/1e9, color='r')
        plt.axvline(frequencies[end]/1e9, color='b')
        plt.axvline(low_edge)
